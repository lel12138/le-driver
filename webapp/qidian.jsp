...
<div id="myPageTop">
    <table border="1">
        ...
        <tr>
            <td colspan="2">
                <input id="tipinput" placeholder="搜索起点："/>
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <a href="/zhongdian">到哪里去</a>
            </td>
        </tr>
    </table>
</div>
    ...
    AMap.plugin(['AMap.PlaceSearch'], function(){
        ...
        function select(e) {
            ...
            function onComplete(data) {
                ...
                $.cookie('lng',data.poiList.pois[0].location.lng);
                $.cookie('lat',data.poiList.pois[0].location.lat);
            }
        ...
        function markerClick(e){
            $("#tipinput").val("从"+e.data.name+"出发");
            //console.log(e.data.location.lng+" "+e.data.location.lat);
            $.cookie('qidian',e.data.name);
            $.cookie('lng',e.data.location.lng);
            $.cookie('lat',e.data.location.lat);
        }
    });
